...
const NewspaperCategory = () => {
  ...
  const { newspaperCategory } = useParams();
  const [newspaper, setNewspaper] = useState('');
  const [thumbnails, setThumbnails] = useState({}); // Store fetched thumbnails
  const [selectedCountry, setSelectedCountry] = useState(null);

  useEffect(() => {
    if (newspaperCategory === 'AlternativeNewspaper'){
      setNewspaper('Alternative Newspaper');
    } else if (newspaperCategory === 'CollegeNewspaper'){
      setNewspaper('College Newspaper');
    } else if (newspaperCategory === 'LocalCountryNewspaper'){
      setNewspaper('Local Newspaper');
    } else if (newspaperCategory === 'WorldNewspaper'){
      setNewspaper('World Newspaper');
    } else {
      setNewspaper('')
    }
  }, [setNewspaper]);
  ...
  return (
    <div className='mainContainer newsCategory'>
